        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          {PROCESS_STEPS.map((step, idx) => (
            <motion.div
              ...
              className="p-6 rounded-xl bg-white border border-slate-200 shadow-sm hover:border-brand-green hover:shadow-md transition-all duration-300 relative group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-mono text-2xl font-bold text-brand-green bg-brand-green/10 px-3 py-1 rounded-md">
                    {step.number}
                  </span>
                  <div className="h-2.5 w-2.5 rounded-full bg-slate-300 group-hover:bg-brand-amber transition-colors" />
                </div>
